...
class PackageVC: UIViewController {
    ...
    {
        if appDelegate.IsfromPackageConfirmation == true
        {
            self.onPagingMenu()
            appDelegate.IsfromPackageConfirmation = false
        }
...
extension PackageVC: PagingViewControllerDataSource {
    
    func pagingViewController(_: PagingViewController, viewControllerAt index: Int) -> UIViewController {
        if index == 0
        {
            return PackagesListVC(title: menu[index])
        }
        else
        {
            return MySubscriptionsVC(title: menu[index])
        }
    }
...
//        return PagingIndexItem(index: index, title: menu[index]) as! T
//    }
//
...
//}
//
extension PackageVC: PagingViewControllerDelegate {
